# Day 3 — Eval Harness Skeleton

## What's built

- **`data/eval_set/eval_questions.json`** — 50 hand-designed questions: 20 single-hop
  (Item 1A/Item 7 prose, all 8 companies), 20 multi-hop-numeric (XBRL-derived, including
  2 genuinely cross-company comparisons and 2 non-trivial derived metrics), 10 out-of-scope
  (10 distinct refusal reasons, not 10 copies of the same one).
...
- **`src/eval/scoring.py`** — Recall@k/MRR with bootstrapped 95% CIs, scored against any
  retriever exposing `.rank(query, company, k)`. Also includes `score_sql_routing_sanity()`
...
- **`scripts/label_eval_set.py`** — interactive local labeling helper for attaching real
  `gold_chunk_ids` to the 20 single-hop questions.
- **`tests/test_scoring.py`** — 12 unit tests, all passing, covering the scoring math and
  both report functions.

## What's NOT done yet — do this before Day 5

**`gold_chunk_ids` are empty for all 20 single-hop questions.** This chat never had access
to your actual `data/processed/` chunks — only the plan doc was uploaded — so I designed
realistic questions and the full labeling infrastructure, but the actual "which chunk_id
answers this" judgment call needs your real corpus. Run:

```bash
python3 scripts/label_eval_set.py
```

against your real `data/processed/<TICKER>/chunks.jsonl` (adjust `CHUNKS_PATH_TEMPLATE` in
that script to match your actual Day 1 output layout first). It surfaces top-8 keyword
candidates per question so labeling 20 questions should be closer to 20-30 minutes than
an hour of re-reading full filing sections.

**`gold_sql_description` fields are plain-English specs, not numbers**, for the same
reason — precise fiscal-year revenue/net-income figures live in your XBRL SQL table, and
I'd rather leave 20 TODOs than hand you numbers I can't verify against your actual data.
These get consumed directly once the SQL tool exists in Phase B (Oct 8) — no extra
